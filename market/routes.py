    ...
    if request.method == "POST":
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"Congratulations!, You have sold {s_item_object.name} back to the market for {s_item_object.price}$", category='success')
            else:
                flash(f"Unfortunately, you don't own {s_item_object.name}!", category='danger')
            
        db.session.commit()  # Save the changes to the database after the transactions are complete.

        # Redirect the user back to the market page after completing the transaction
